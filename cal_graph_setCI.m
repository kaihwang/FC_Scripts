...
for n = 1:length(Adj.Matrix_Full) % loop through parcellations.. although now just using 1.
   ...
   for T = 0.01:0.005:0.25; %density threshold
       ...
       Graph.P{n}(i,:) = participation_coef(threshold_proportional(M,T), WashU297ROI_CI);
       Graph.WMD{n}(i,:) = module_degree_zscore(weight_conversion(threshold_proportional(M,T),'binarize'), WashU297ROI_CI);
       Graph.Full_Q{n}(i) = Q;
       Graph.Full_E{n}(i) = E;
...
for n = 1:length(Adj.Matrix_Full)
   ...
   for T =  0.01:0.005:0.25;
       ...
       Graph.Right_WMD{n}(i,:) = module_degree_zscore(weight_conversion(threshold_proportional(M,T),'binarize'), WashU297ROI_CI(146:297));
       Graph.Right_Q{n}(i) = Q;
       Graph.Right_E{n}(i) = E;
...
for n = 1:length(Adj.Matrix_Full)
   ...
   for T = 0.01:0.005:0.25;
       ...
       Graph.Left_WMD{n}(i,:) = module_degree_zscore(weight_conversion(threshold_proportional(M,T),'binarize'), WashU297ROI_CI(1:145));
       Graph.Left_Q{n}(i) = Q;
       Graph.Left_E{n}(i) = E;
