...
for sub = Tha_Subj
    fn = strcat('/home/despo/kaihwang/Rest/Graph/g_',num2str(sub),'.mat');
    load (fn);
    %dlmwrite('176_40.node', Craddock_Coor_40,'delimiter','\t')
    for p = Partitions %1: length(Graph.Full_Q)
...
for t=Partitions;
    ...
    hold on;
    H2=shadedErrorBar(T,squeeze(Out_Module_Weight_Control_R(t,:,:)),{@mean, @ste},{'--','LineWidth',3,'Color',rgb('lightgrey')},1);
...
for t=[12, 20];
    ...
    hold on;
    H2=shadedErrorBar(T,squeeze(Within_Module_Weight_Control_R(t,:,:)),{@nanmean, @ste},{'--','LineWidth',3,'Color',rgb('lightgrey')},1);
...
for t=[12, 20];
    ...
    hold on;
    H2=shadedErrorBar(T,squeeze(CC_Control_R(t,:,:)),{@nanmean, @ste},{'--','LineWidth',3,'Color',rgb('lightgrey')},1);
