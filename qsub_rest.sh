...
for Subject in 128 162 163 168 176; do
		...
		qsub -V -M kaihwang -m e -l mem_free=8G -e ~/tmp -o ~/tmp tmp/q_proc_th_HCPpreproc_reg_bp_${Subject}.sh
...
for Subject in b116 b117 b120 b121 b122 b138 b143 b144 b153; do
		...
		qsub -V -M kaihwang -m e -l mem_free=8G -e ~/tmp -o ~/tmp tmp/q_proc_bg_HCPpreproc_reg_bp_${Subject}.sh
...
for Subject in 114 117 119 203 205 207 209 211 213 215 217 219 116 118 201 204 206 208 210 212 214 216 218 220; do
		...
		qsub -V -M kaihwang -m e -l mem_free=8G -e ~/tmp -o ~/tmp tmp/q_proc_c_HCPpreproc_reg_bp_${Subject}.sh
